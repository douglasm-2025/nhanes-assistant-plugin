...
The plugin auto-loads canonical survey helpers (`r_helpers/nhanes_survey.R`, also readable at `config://nhanes_survey`) before every `execute_r_script` run. **Build the survey design with `nh_design()`** — do not hand-roll `svydesign()` for a standard NHANES design. (Escape hatch: if a task genuinely needs a design `nh_design()` cannot express — replicate weights, a non-standard subsample, or older cycles with different design variables — build it explicitly with `svydesign()` and briefly note why.) Estimate on the design with `nh_mean()`/`nh_diff()` for means and between-group differences, and the appropriate survey function for anything else (`svyciprop` for prevalence with a logit CI, `svyglm` for regression, `svyby` for by-group, `svyquantile`, `svycontrast`). This keeps every analysis correct by construction and near-identical run to run (reproducible and auditable).
